"""Build a curated benchmark dataset under ``data/final/``.
...
# --- Run configuration (edit these, then: python scripts/build_final_dataset.py) ---
...
def iter_processed_jsonl(processed_root: Path) -> Iterator[tuple[str, Path]]:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=f"Max rows per topic per benchmark (default: {DEFAULT_MAX_PER_TOPIC}).",
    )
    ...
    if args.max_per_topic < 1:
        raise SystemExit("--max-per-topic must be at least 1")
    ...
    for benchmark, path in iter_processed_jsonl(args.input):
    ...
    for benchmark in sorted(by_benchmark):
        ...
        print(
            ...
            f"(max {args.max_per_topic} per topic)"
        )
    ...
    print(
        ...
        f"{total_out} row(s) out (max {args.max_per_topic} per topic per benchmark)"
    )
